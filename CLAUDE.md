...
- `.claude/skills/` — 실행 워크플로우 (/plan, /workstream, /self-review, /codex-review, /security-review)

## 핵심 플로우

rules/는 모든 작업에 항상 적용된다.

기본 엔진은 Claude Code이며, 다음 게이트는 필수다.

1. **Plan (Codex 필수)** — `/plan`
2. **Build (Claude 기본/필수)** — `/workstream` 또는 직접 구현
3. **Review (Codex 필수)** — `/codex-review`
4. Security review — `/security-review` (권장, 민감 변경 시 필수)

`/plan` 또는 `/codex-review`가 없으면 작업을 완료로 보고하지 않는다.

### workstream 기반 (기능 개발, 대규모 변경)

1. `/plan` (Codex) — 설계 및 구현 계획 수립
2. `/workstream` (Claude) — 계획 실행 (체크리스트 기반)
   - feature 단위마다 `/self-review` 실행
3. workstream 완료 리뷰 — `/codex-review` (필수) → `/security-review`

### 단순 작업 (버그 수정, 설정 변경, 소규모 개선)

규모가 작아도 Plan/Review 게이트는 동일하다.
- 최소: `/plan` → 구현(Claude) → `/codex-review`
